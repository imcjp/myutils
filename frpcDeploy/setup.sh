...
# 提示用户关于配置文件的信息
echo "在 ${current_dir}/${dirName} 下有配置示例文件 frpc.toml.example。" > "${current_dir}/${dirName}/readme.txt"
echo "您可以参考此示例文件编写您的配置文件，例如 XXX.toml。" >> "${current_dir}/${dirName}/readme.txt"
echo "然后，通过服务 frpc@XXX.service 运行 frpc，其中 XXX 指代您的配置文件名称。" >> "${current_dir}/${dirName}/readme.txt"
echo "请确保您的配置文件后缀为 *.toml。" >> "${current_dir}/${dirName}/readme.txt"

# 提示用户如何使用 frpc@.service
echo "使用方法：" >> "${current_dir}/${dirName}/readme.txt"
echo "1. 编写配置文件并放置在 ${current_dir}/${dirName} 目录下，例如 ${current_dir}/${dirName}/XXX.toml。" >> "${current_dir}/${dirName}/readme.txt"
...
echo "请替换 XXX 为您的配置文件名称，不包含.toml后缀。" >> "${current_dir}/${dirName}/readme.txt"
